...
test('終了した仲間会話は公開導線から外し、互換用UIだけを保持する', async () => {
  ...
  assert.doesNotMatch(screen, /CHARACTER_(?:TALK|DAILY|GRIEVANCE)_PATTERN_COUNT/)
  ...
  assert.doesNotMatch(screen, /CHARACTER_DAILY_QUESTIONS\.length/)
  ...
  assert.doesNotMatch(screen, /主人公は画面外|手・姿・影・反射/)
