...
test('全18教材・16,355項目を一覧行へ重複も欠落もなく変換する', () => {
  assert.equal(LEARNING_CONTENTS.length, 18)
  assert.equal(learningContentCatalogTotal(LEARNING_CONTENTS), 16_355)
  assert.deepEqual(
...
test('英単語と指定9カテゴリの全12,082項目を学習・テストの連続スワイプ対象にする', () => {
  ...
  assert.equal(contents.reduce((sum, content) => sum + content.items.length, 0), 12_082)
