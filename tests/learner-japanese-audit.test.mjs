...
test('学習者向け日本語は画面・部品・自動生成文を全件監査する', async () => {
  ...
  assert.equal(result.learnerFiles, 250)
  assert.equal(result.learnerJapaneseEntries, 12692)
  assert.equal(result.learnerUniqueJapaneseEntries, 10078)
  assert.equal(result.sourceFiles, 587)
  assert.equal(result.sourceJapaneseEntries, 129152)
