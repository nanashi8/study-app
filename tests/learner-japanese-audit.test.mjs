...
test('学習者向け日本語は画面・部品・自動生成文を全件監査する', async () => {
  ...
  assert.equal(result.learnerFiles, 216)
  assert.equal(result.learnerJapaneseEntries, 12050)
  assert.equal(result.learnerUniqueJapaneseEntries, 9671)
  assert.equal(result.sourceFiles, 493)
  assert.equal(result.sourceJapaneseEntries, 105736)
