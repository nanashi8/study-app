...
test('学習者向け日本語は画面・部品・自動生成文を全件監査する', async () => {
  ...
  assert.equal(result.learnerFiles, 255)
  assert.equal(result.learnerJapaneseEntries, 12738)
  assert.equal(result.learnerUniqueJapaneseEntries, 10103)
  assert.equal(result.sourceFiles, 595)
  assert.equal(result.sourceJapaneseEntries, 129706)
