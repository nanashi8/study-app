...
test('学習者向け日本語は画面・部品・自動生成文を全件監査する', async () => {
  ...
  assert.equal(result.learnerFiles, 217)
  assert.equal(result.learnerJapaneseEntries, 12061)
  assert.equal(result.learnerUniqueJapaneseEntries, 9655)
  assert.equal(result.sourceFiles, 494)
  assert.equal(result.sourceJapaneseEntries, 105747)
