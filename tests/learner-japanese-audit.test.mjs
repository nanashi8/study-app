...
test('学習者向け日本語は画面・部品・自動生成文を全件監査する', async () => {
  ...
  assert.equal(result.learnerFiles, 218)
  assert.equal(result.learnerJapaneseEntries, 12100)
  assert.equal(result.learnerUniqueJapaneseEntries, 9680)
  assert.equal(result.sourceFiles, 495)
  assert.equal(result.sourceJapaneseEntries, 105786)
