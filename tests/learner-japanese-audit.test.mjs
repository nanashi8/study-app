...
test('学習者向け日本語は画面・部品・自動生成文を全件監査する', async () => {
  ...
  assert.equal(result.learnerFiles, 242)
  assert.equal(result.learnerJapaneseEntries, 12591)
  assert.equal(result.learnerUniqueJapaneseEntries, 10042)
  assert.equal(result.sourceFiles, 569)
  assert.equal(result.sourceJapaneseEntries, 116148)
