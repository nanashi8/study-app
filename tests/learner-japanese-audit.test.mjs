...
test('学習者向け日本語は画面・部品・自動生成文を全件監査する', async () => {
  ...
  assert.equal(result.learnerJapaneseEntries, 12509)
  assert.equal(result.learnerUniqueJapaneseEntries, 9710)
  assert.equal(result.sourceFiles, 610)
  assert.equal(result.sourceJapaneseEntries, 139708)
