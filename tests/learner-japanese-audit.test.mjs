...
test('学習者向け日本語は画面・部品・自動生成文を全件監査する', async () => {
  ...
  assert.equal(result.learnerFiles, 257)
  assert.equal(result.learnerJapaneseEntries, 12801)
  assert.equal(result.learnerUniqueJapaneseEntries, 9944)
  assert.equal(result.sourceFiles, 641)
  assert.equal(result.sourceJapaneseEntries, 147609)
