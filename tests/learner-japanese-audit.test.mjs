...
test('学習者向け日本語は画面・部品・自動生成文を全件監査する', async () => {
  ...
  assert.equal(result.learnerFiles, 252)
  assert.equal(result.learnerJapaneseEntries, 12690)
  assert.equal(result.learnerUniqueJapaneseEntries, 10073)
  assert.equal(result.sourceFiles, 591)
  assert.equal(result.sourceJapaneseEntries, 129482)
