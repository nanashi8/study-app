...
test('全長文・長い一文・文学の意味フレーズと内部SVOCMを監査する', () => {
  ...
  assert.equal(audit.reading.phraseCount, 6883)
  assert.equal(audit.reading.meaningPhraseCount, 3335)
  assert.equal(audit.reading.meaningMultiRoleCount, 2059)
...
test('全規則と全文を監査確認済みにし、台帳外変更は別状態として扱う', () => {
  ...
  assert.equal(audit.reading.confirmedPhraseCount, 6883)
