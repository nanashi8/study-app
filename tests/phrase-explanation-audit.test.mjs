...
test('全長文・長い一文・文学の意味フレーズと内部SVOCMを監査する', () => {
  ...
  assert.equal(audit.reading.phraseCount, 6882)
  assert.equal(audit.reading.meaningPhraseCount, 3840)
  assert.equal(audit.reading.meaningMultiRoleCount, 2049)
  assert.equal(audit.reading.grammarBlockCount, 2420)
  assert.equal(audit.reading.correctionDecisionCount, 940)
  assert.equal(audit.reading.appliedCorrectionCount, 940)
  assert.deepEqual(audit.reading.connectorClosureReview, {
    candidateCount: 147,
    backReferenceCount: 63,
    alreadyClearCount: 84,
  })
...
test('全規則と全文を監査確認済みにし、台帳外変更は別状態として扱う', () => {
  ...
  assert.equal(audit.reading.manuallyReviewedSentenceCount, 990)
  ...
  assert.equal(audit.reading.confirmedPhraseCount, 6882)
...
test('明示台帳外の新規文と、既存文のJA・role・文法ブロック変更はreview-neededへ戻る', () => {
  assert.equal(Object.keys(READING_MANUAL_REVIEW_LEDGER).length, 990)
  assert.equal(
