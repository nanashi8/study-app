...
test('古典単語の活用表：助動詞の活用表は学校文法の28語と上代の3語すべてを持つ', () => {
  const names = Object.keys(AUXILIARY_TABLES).map((name) => name.replace(/（[^）]*）$/u, ''))
  for (const name of ['る', 'らる', 'す', 'さす', 'しむ', 'ず', 'じ', 'む', 'むず', 'まし', 'まほし', 'たし', 'き', 'けり', 'つ', 'ぬ', 'たり', 'り', 'けむ', 'らむ', 'らし', 'めり', 'なり', 'べし', 'まじ', 'ごとし', 'ゆ', 'らゆ', 'ふ']) {
    assert.ok(names.includes(name), `助動詞「${name}」の活用表がない`)
  }
  assert.equal(Object.keys(AUXILIARY_TABLES).length, 31)
})
