...
test('問題別解説と選択肢別根拠を混同せず、記録済み台帳の監査数を残す', async () => {
  ...
  assert.equal(ledger.extendedReadingDetail.baselineCoveragePercent, 21.98)
  assert.equal(ledger.extendedReadingDetail.combinedCoveredVocabulary, 3_951)
  assert.equal(ledger.extendedReadingDetail.combinedCoveragePercent, 44.36)
...
test('全選択式教材の正解・全誤答・わからない経路数を教材別に残す', async () => {
  ...
  assert.equal(paths.questionCount, 4_432)
  assert.equal(paths.displayedChoiceCount, 17_354)
  assert.equal(paths.unknownPathCount, 4_432)
  assert.equal(paths.answerPathCount, 21_786)
  assert.deepEqual(
