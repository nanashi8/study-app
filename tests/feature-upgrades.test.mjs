...
test('語源の全公開入口は語根の暗記・テスト・一覧と通常の単語暗記をそろえる', () => {
  ...
  assert.match(wordBits, /関連する\{card\.coverageIds\.length\}語/)
  assert.doesNotMatch(wordBits, /確認済み・関連する/)
...
test('学習分析は計算方法・記録数・図表・個別助言・値の限界を一画面で説明する', () => {
  ...
  // 学び方の中身は残し、論文の書誌・リンクや出典確認の説明は生徒の画面に載せない。
  assert.match(analytics, /答えを見る前に、自力で一度思い出す。/)
  assert.doesNotMatch(analytics, /doi\.org|citation|参考文献|出典を確認した|科学的根拠と限界|このアプリの問題結果から求めた目安/)
...
test('メニューの全教材・個人機能は公開ルートに存在し、履歴消去は一つの選択画面へ集約する', () => {
  ...
  assert.equal(publicScreens.size, 74)
