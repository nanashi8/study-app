...
test('全長文・全文・全ブロックに講師監修の語順訳シナリオが対応する', () => {
  ...
  assert.equal(meaningPhraseCount, 4880, '全4,880件の学習者向け意味フレーズを対象にする')
  assert.equal(meaningMultiRoleCount, 1973, 'SVOCMを内部に複数含む1,973件も一つの意味フレーズとして保つ')
})
