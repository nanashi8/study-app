...
test('全長文・全文・全ブロックに講師監修の語順訳シナリオが対応する', () => {
  ...
  assert.equal(phrasePairCount, 6882, '全6,869ブロック内役割単位を英語フレーズと直訳の組にする')
  assert.equal(meaningPhraseCount, 4176, '全4,176件の学習者向け意味フレーズを対象にする')
  assert.equal(meaningMultiRoleCount, 2081, 'SVOCMを内部に複数含む2,081件も一つの意味フレーズとして保つ')
})
