...
test('全長文・全文・全ブロックに講師監修の語順訳シナリオが対応する', () => {
  ...
  assert.equal(phrasePairCount, 6881, '全6,869ブロック内役割単位を英語フレーズと直訳の組にする')
  assert.equal(meaningPhraseCount, 4295, '全4,295件の学習者向け意味フレーズを対象にする')
  assert.equal(meaningMultiRoleCount, 2076, 'SVOCMを内部に複数含む2,076件も一つの意味フレーズとして保つ')
})
