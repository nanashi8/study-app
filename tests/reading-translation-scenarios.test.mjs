...
test('全長文・全文・全ブロックに講師監修の語順訳シナリオが対応する', () => {
  ...
  assert.equal(meaningPhraseCount, 4678, '全4,678件の学習者向け意味フレーズを対象にする')
  assert.equal(meaningMultiRoleCount, 2025, 'SVOCMを内部に複数含む2,025件も一つの意味フレーズとして保つ')
})
