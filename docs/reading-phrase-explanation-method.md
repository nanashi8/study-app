...
Readerは、文型監査用の `phraseSequence` を直接フレーズカードとして表示しない。そこから作った `meaningPhraseSequence` を主表示・音声・下段ブロックへ共通投影し、SVOCMは各カード内部の小さな注釈として示す。内部単位のfingerprintは文法判断の退行を検知し、意味フレーズ監査は不自然な細切れ・原文復元・対応日本語・音声を別に検知する。文上部の見取り図はカード境界を括弧へ変換せず、節・句の入れ子構造として独立表示する。
