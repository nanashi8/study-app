...
import { etymologyCompanions } from '../lib/etymologyCompanions.js'
import {
...
export function EtymologyKnowledgeAnswer({ pack, words }) {
  const guide = etymologyMeaningGuideFor(pack)
  // カードの語だけだと1語しか出ないことがある。つながりの強い順に、
  // 一緒に覚えられる語を必ず添える（理由も一緒に出す）。
  const companions = useMemo(() => etymologyCompanions(pack), [pack?.id])
  ...
  return (
    <div className="space-y-5" data-etymology-learning-flow>
      ...
      <section className="space-y-3" aria-labelledby="etymology-step-related">
        ...
        {companions.length > 0 && (
          <div className="rounded-2xl bg-violet-50/70 p-3 ring-1 ring-violet-100" data-etymology-companions>
            <p className="text-xs font-extrabold text-violet-700">
              一緒に覚えられる語（{companions.length}語）
            </p>
            <p className="mt-0.5 text-[11px] font-bold leading-relaxed text-ink/45">
              このカードの語とつながりのある語です。かっこの中がつながり方です。
            </p>
            <ul className="mt-2 grid gap-1.5 sm:grid-cols-2">
              {companions.map(({ word, reason }) => (
                <li key={word.id} className="min-w-0 rounded-xl bg-white px-2.5 py-2 ring-1 ring-violet-100">
                  <div className="flex items-baseline gap-1.5">
                    <span className="font-display text-sm font-extrabold text-ink">{word.word}</span>
                    <span className="min-w-0 flex-1 truncate text-[11px] font-bold text-ink/55">
                      {word.meanings?.slice(0, 2).join('・') || word.meaning}
                    </span>
                  </div>
                  <p className="mt-0.5 text-[10px] font-extrabold text-violet-600">{reason}</p>
                </li>
              ))}
            </ul>
          </div>
