...
//
// 復習の枠は「まだ」「不正解」の語から、未修の枠は未修の語から出す。どちらも足りない分は
// 「覚えた」「正解」の語で埋める（復習の枠は点数の低い順、未修の枠は出題順）。
// 途中の段（復習寄り・半々・未修寄り）は、それでも足りない枠をもう一方で補う。
// 両端の「復習だけ」は未修の語を、「未修だけ」は「まだ」「不正解」の語を出さない（そのぶん少なく出す）。
// どの段でも、何度も「まだ」「不正解」をくり返している語（苦手）は、未修の語と混ぜる前に先頭へまとめて出す。
 ...
 * 「未修だけ」が空になるのは、未修も「覚えた」「正解」の語もなく、「まだ」「不正解」の語だけが残るとき。
 * 「復習だけ」が空になるのは、まだ何も学んでいないとき。出さない理由と、バーで続けられることを伝える。
 */
export function vocabMixEmptyNotice(value) {
  ...
  if (share === 1) {
    return {
      title: '未修の単語は残っていません',
      detail: '出題バランスが「未修だけ」なので、「まだ」「不正解」の単語は出していません。下のバーで割合を変えると、その単語も出せます。',
    }
  }
  if (share === 0) {
    return {
