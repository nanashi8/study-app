...
const RAW = [
  ['state', '名', 'pre1', '状態・国家・州・述べる', 'The old house was in a bad state.', 'その古い家はひどい状態だった。', 'ラテン status(立っていること)→ stand と同系。', { syn: [{ w: 'condition', m: '状態' }, { w: 'nation', m: '国家' }], fam: [{ w: 'statement', m: '声明' }], field: '一般' }],
  ['renew', '動', 'pre1', '一新する・更新する・再開する', 'I need to renew my passport before our trip to Canada.', 'カナダ旅行の前に、パスポートを更新しなければならない。', 're(再)+古フランス novel(新しい)→ new と同系。', { syn: [{ w: 'update', m: '更新する' }, { w: 'restore', m: '回復する' }], ant: [{ w: 'cancel', m: '解約する' }], fam: [{ w: 'renewable', m: '再生可能な' }], field: '一般' }],
  ['settler', '名', '2', '入植者・定住者', 'The first settlers in the valley built their houses from logs.', 'その谷の最初の入植者たちは、丸太で家を建てた。', 'settle(定住する)+ -er', { fam: [{ w: 'settle', m: '定住する' }], syn: [{ w: 'colonist', m: '植民者' }, { w: 'pioneer', m: '開拓者' }], ant: [{ w: 'nomad', m: '遊牧民' }], field: '社会' }],
  ...
  ['country', '名', 'pre1', '国・国土・田舎', 'Canada is a very large country.', 'カナダはとても大きな国だ。', '古フランス contrée(地方)。', { syn: [{ w: 'nation', m: '国家' }, { w: 'state', m: '国' }], ant: [{ w: 'city', m: '都市' }], fam: [{ w: 'countryside', m: '田園' }], field: '政治' }],
  ...
  ['exterior', '名', 'pre1', '外部・外面・外側の', 'The exterior of the house was painted light blue.', 'その家の外側は水色に塗られていた。', 'ラテン exterior(より外の)。', { syn: [{ w: 'outside', m: '外側' }, { w: 'facade', m: '外観' }], ant: [{ w: 'interior', m: '内部' }], field: '一般' }],
  ...
  ['inside', '名', 'pre1', '内側・内部・中に', 'The inside of this cake is chocolate.', 'このケーキの内側はチョコレートになっている。', 'in(中)+side(側)。', { syn: [{ w: 'interior', m: '内部' }, { w: 'core', m: '中心' }], ant: [{ w: 'outside', m: '外側' }], field: '一般' }],
  ['notices', '名', 'pre1', '通知・掲示・注目', 'post notices', '掲示を貼る', 'notice(通知)の複数。', { syn: [{ w: 'announcement', m: '発表' }, { w: 'warnings', m: '警告' }], fam: [{ w: 'notify', m: '通知する' }], field: 'メディア' }],
  ...
  ['novelist', '名', 'pre1', '小説家', 'The novelist spent five years writing her latest book.', 'その小説家は最新作を書くのに5年を費やした。', 'novel(小説)+ -ist。', { syn: [{ w: 'author', m: '作家' }, { w: 'writer', m: '著述家' }], fam: [{ w: 'novel', m: '小説' }], field: '文学' }],
  ['nuanced', '形', 'pre1', '微妙な・ニュアンスのある', 'The actor gave a nuanced performance as a kind but lonely father.', 'その俳優は、優しいけれど孤独な父親を微妙なニュアンスに富んだ演技で表現した。', 'nuance(微妙な差)+ -d。', { syn: [{ w: 'subtle', m: '繊細な' }, { w: 'sophisticated', m: '洗練された' }], ant: [{ w: 'simplistic', m: '単純すぎる' }], fam: [{ w: 'nuance', m: '微妙な差' }], field: '言語' }],
