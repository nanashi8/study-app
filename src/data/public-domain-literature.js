...
// marked は漢文の訓読文（返り点付き）。返り点の要らない一文でも、白文と同じ形で必ず持たせる。
const scene = (original, translation, guide, speech = null, marked = null) =>
  Object.freeze({ original, translation, guide, speech, marked })

const source = (label, url, checkedOn) => Object.freeze({ label, url, checkedOn })

// 漢文は朗読の区切りごとにも返り点付きで見せる。場面の訓読文を、区切りの白文と
// 同じ字数で切り分けるので、連結すれば必ず元の訓読文へ戻る。
  ...
  Object.freeze({
    ...
    scenes: Object.freeze([
      ...
      scene(
        ...
        '有㆑朋自遠方來、不㆓亦樂㆒乎？人不㆑知而不㆑慍、不㆓亦君子㆒乎？',
      ),
      scene(
        '子曰：「溫故而知新、可以爲師矣。」',
