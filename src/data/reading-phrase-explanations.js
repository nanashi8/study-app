...
const guides = [
  ...
  guide(
    ...
    [
      phrase('This evidence', 'S', 'この根拠は'),
      phrase(
        'makes it easier',
        ['V', 'O', 'C'],
        '〜することをより簡単にします（it の中身は次へ）',
        'it は形式目的語で、中身は後ろの to improve … or decide … です。make it easier to do で「〜するのをより簡単にする」。',
      ),
      phrase('to improve a design', ['V', 'O'], '設計を改善すること'),
      phrase(
        'or decide',
        ['LINK', 'V'],
        'あるいは判断すること（内容は次へ）',
        'decide は to improve と to を共有しています（(to) decide）。',
        { displayEn: 'or (to) decide' },
      ),
      phrase(
        'that a simpler solution would work better',
        ['LINK', 'S', 'V', 'M'],
        'より単純な解決策のほうがうまくいく、と',
        'that 以下は decide の目的語になる名詞節です。would は控えめな推量です。',
      ),
    ],
  ),
  guide(
    ...
    '長い主語、受動態のあとで、less by A than by B の二つの by what … を、比べる相手ごとのまとまりで読みます。',
    [
      phrase('The integrity of public memory', 'S', '公共的記憶の健全さは'),
      phrase('is then shaped', 'V', 'そうなると形づくられます（何によってかは次へ）'),
      phrase('less by what is available', ['M', 'V'], '入手できるものによってというより'),
      phrase('than by what is repeatedly presented as relevant', ['LINK', 'M', 'V', 'C'], 'むしろ、繰り返し関連のあるものとして示されるものによって'),
    ],
  ),
  guide(
    ...
    '条件節、even が強める主語、prevent O from -ing、ability を説明する不定詞と from what の節を、意味のまとまりで保ちます。',
    [
      phrase('If that practice declines,', ['LINK', 'S', 'V'], 'もしその実践が衰えれば'),
      phrase('even perfect archives', ['M', 'S'], '完璧な記録保管所でさえ'),
      phrase('will not prevent', 'V', '防げないでしょう（何をかは次へ）'),
      phrase('societies from losing their ability', ['O', 'M'], '社会が自分たちの能力を失うのを'),
      phrase('to learn from what they once knew', ['V', 'M', 'S'], 'かつて知っていたことから学ぶ（能力を）'),
    ],
  ),
]
