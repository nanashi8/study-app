...
const RAW = [
  ...
  ['revere', '動', '1', '崇敬する・あがめる', 'They revere their ancestors.', '彼らは祖先をあがめる。', 'ラテン re+vereri(おそれる)。', { syn: [{ w: 'worship', m: '崇拝する' }, { w: 'venerate', m: 'あがめる' }], ant: [{ w: 'despise', m: '軽蔑する' }], fam: [{ w: 'reverence', m: '崇敬' }, { w: 'reverent', m: '敬虔な' }], field: '宗教' }],
  ...
  ['proficiency', '名', '1', '熟達・堪能', 'The job requires proficiency in both English and Chinese.', 'その仕事には、英語と中国語の両方に堪能であることが求められる。', 'ラテン proficere(進歩する)→ profit と同系。', { syn: [{ w: 'skill', m: '技能' }], ant: [{ w: 'incompetence', m: '無能' }], fam: [{ w: 'proficient', m: '堪能な' }], field: '教育' }],
  ...
  ['sanctuary', '名', '1', '聖域・避難所・保護区', 'The island is a sanctuary for rare seabirds.', 'その島は珍しい海鳥の保護区になっている。', 'ラテン sanctus(神聖な)→ saint と同系。', { syn: [{ w: 'refuge', m: '避難所' }, { w: 'haven', m: '安息所' }], field: '環境' }],
  ...
  // 上級形容詞
  ['arduous', '形', '1', '骨の折れる・困難な', 'Carrying supplies up the mountain was an arduous task.', '物資を山の上まで運ぶのは骨の折れる仕事だった。', 'ラテン arduus(険しい)。', { syn: [{ w: 'difficult', m: '困難な' }, { w: 'strenuous', m: '骨の折れる' }], ant: [{ w: 'easy', m: '容易な' }], field: '性質・状態' }],
  ['compelling', '形', '1', '説得力のある・引き込まれる', 'The lawyer presented compelling evidence that her client was innocent.', '弁護士は、依頼人が無実だという説得力のある証拠を示した。', 'compel(強いる)+ -ing。', { syn: [{ w: 'persuasive', m: '説得力のある' }], ant: [{ w: 'weak', m: '弱い' }], field: '性質・状態' }],
  ['fervent', '形', '1', '熱烈な・熱心な', 'My aunt is a fervent supporter of the local soccer team.', 'おばは地元のサッカーチームの熱烈な支持者だ。', 'ラテン fervere(沸騰する)→ fever と同系。', { syn: [{ w: 'passionate', m: '情熱的な' }, { w: 'ardent', m: '熱烈な' }], ant: [{ w: 'indifferent', m: '無関心な' }], fam: [{ w: 'fervor', m: '熱情' }], field: '性質・状態' }],
  ['haphazard', '形', '1', '行き当たりばったりの・無計画な', 'The books were stacked in a haphazard way, so nothing was easy to find.', '本が無計画に積まれていたので、何を探すのも大変だった。', 'hap(偶然)+hazard(運)。', { syn: [{ w: 'random', m: '無作為の' }, { w: 'disorganized', m: '雑然とした' }], ant: [{ w: 'systematic', m: '体系的な' }], field: '性質・状態' }],
  ['incessant', '形', '1', '絶え間ない・ひっきりなしの', 'The incessant ringing of her phone made it impossible to concentrate.', '絶え間なく鳴る電話のせいで、彼女は集中できなかった。', 'ラテン in(否定)+cessare(やむ)→ cease と同系。', { syn: [{ w: 'constant', m: '絶え間ない' }, { w: 'ceaseless', m: 'やむことのない' }], ant: [{ w: 'intermittent', m: '断続的な' }], field: '性質・状態' }],
  ['meager', '形', '1', '乏しい・わずかな', 'The farmers lived on a meager income after the poor harvest.', '不作の後、農家の人々はわずかな収入で暮らした。', 'ラテン macer(やせた)。', { syn: [{ w: 'scanty', m: '乏しい' }, { w: 'paltry', m: 'ささいな' }], ant: [{ w: 'abundant', m: '豊富な' }], field: '性質・状態' }],
  ...
  ['ruthless', '形', '1', '無慈悲な・容赦ない', 'The ruthless king punished anyone who disagreed with him.', 'その無慈悲な王は、自分に反対する者をだれでも罰した。', 'ruth(哀れみ)+ -less。', { syn: [{ w: 'merciless', m: '無慈悲な' }, { w: 'brutal', m: '残忍な' }], ant: [{ w: 'merciful', m: '慈悲深い' }], field: '性質・状態' }],
