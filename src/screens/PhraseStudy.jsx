  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-4">
        <div key={item.id} onClick={() => !flipped && setFlipped(true)} className="animate-pop-in rounded-[2rem] bg-white p-6 shadow-card">
          <div className="flex justify-between">
            <Chip color={level.color}>英検{level.label}</Chip>
            <Chip color={item.kind === 'syntax' ? '#8b5cf6' : '#0ea5e9'}>{item.kind === 'syntax' ? '構文' : '熟語'}</Chip>
          </div>
          ...
          {!flipped ? (
            <div className="mt-6 flex flex-col items-center gap-2 rounded-2xl border-2 border-dashed border-brand-200 py-8 text-brand-400">
              <span className="text-sm font-extrabold">タップして意味と例文を見る</span>
      ...
      <div className="shrink-0 border-t border-brand-100 bg-white/90 p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] backdrop-blur">
        {!flipped ? (
          <Button full size="lg" onClick={() => setFlipped(true)}>答えを見る</Button>
        ) : (
