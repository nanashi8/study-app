...
- **単語モード（深く作り込み済み）**
  ...
  - 🔊 **暗記カード**（タップで発音・意味・例文。確認済みの語源カードがある語だけ導線を表示）
  - **3択＋「わからない」テスト**（即時フィードバック）
  - **単語詳細** … 手動監査済みの語源カードと、明示的に確認した関連語だけへジャンプ
  - **マイ単語リスト**に保存して、まとめて暗記・テスト
  - 簡易 **間隔反復（SRS）** で苦手語を自動で復習に出題
  - 収録：英検5級〜1級・大学受験対応の**8,869語**。全語に**分野(field)タグ**と発音記号(IPA)つき（IPAは CMU Pronouncing Dictionary から自動生成）
  - **語源カード**：出典と全リンクを手動監査した196枚を、語根そのものの**暗記・3択テスト・一覧**で学習（記録は語源専用SRS）。1,977語・延べ2,019リンクだけを公開し、未承認の自由記述は表示しない
...
- **長文モード**
  ...
  - **まとめ**で出現語をまとめて暗記・テスト・マイ単語に保存。読了マーク付き
- **熟語・構文モード**
